  ...
  return (
    <nav style={{backgroundColor: `${color}`}} className=' fixed left-0 top-0 w-full z-10 ease-in duration-300'>
      <div className=' max-w-[1240px] m-auto flex justify-between items-center p-4 text-white'>
        ...
        {/* Mobile Button */}
        <div onClick={handleNav} className=' sm:hidden block z-20'>
          {nav ? <AiOutlineClose size={20} style={{color:`${textColor}`}} />  : <AiOutlineMenu  style={{color:`${textColor}`}} size={20}/>}
        </div>
        {/* mobile menu */}
        <nav onClick={handleNav} className={nav ? ' sm:hidden absolute top-0 left-0 right-0 bottom-0 flex justify-center items-center w-full h-screen bg-black text-center ease-in duration-300' : ' sm:hidden absolute top-0 left-[-100%] right-0 bottom-0 flex justify-center items-center w-full h-screen bg-black text-center ease-in duration-300'}>
          <ul>
            <li onClick={handleNav} className=' p-4 text-4xl'><Link href={'/'}>Home</Link></li>
            <li onClick={handleNav} className=' p-4 text-4xl'><Link href='#gallery'>Gallery</Link></li>
            <li onClick={handleNav} className=' p-4 text-4xl'><Link href='/work'>Work</Link></li>
            <li onClick={handleNav} className=' p-4 text-4xl'><Link href='/contact'>Contact</Link></li>
          </ul>
        </nav>
      </div>
    </nav>
